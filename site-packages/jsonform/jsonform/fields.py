...
__all__ = [
    'Field', 'StringField', 'IntegerField', 'FloatField', 'BooleanField', 'DateTimeField',
    'DateField', 'TimeField', 'ListField', 'SetField', 'EnumField', 'BytesField'
]
...
class Field(ABC):
    ...
    def __init__(self, condition=AnyValue(), optional: bool = False, default=None, init=False):
...
class DictField(Field):
    def __init__(self, key_field, val_field, default=set, **kwargs):
    ...
    def data(self):
        if not self.data_:
            self.data_ = {key.data: val.data for key, val in self._data.items()}
        return self.data_

    def process_data(self, value):
        if not isinstance(value, dict):
            raise FieldError('invalid dict')

        e = FieldError()
        for key, val in value:
            key_field = self.key_field.bind()
            val_field = self.val_field.bind()
            try:
                key_field.process_data(key)
                val_field.process_data(val)
            except FieldError as e_:
                e[key] = e_.error
            self._data[key_field] = val_field
        if e:
            raise e
        super().process_data(value)
