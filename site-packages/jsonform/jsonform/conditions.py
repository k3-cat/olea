...
__all__ = [
    'BaseCondition', 'AnyValue', 'All', 'Any', 'InRange', 'LenInRange', 'In', 'NotIn', 'Email',
    'Regexp'
]
...
class In(BaseCondition):
    ...
    def check(self, field):
        ...
        if len(data | self.values) > len(self.values):
            raise FieldError(e='not allow: ' + ', '.join({str(val)
...
class Regexp(BaseCondition):
    ...
    def check(self, field):
        ...
        if not match:
            raise FieldError(
                e=f'input does not match the pattern {self.regex}' if not self.msg else self.msg)
